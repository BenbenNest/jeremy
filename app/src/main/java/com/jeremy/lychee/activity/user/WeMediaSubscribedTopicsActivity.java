...
public class WeMediaSubscribedTopicsActivity extends SlidingActivity {
    ...
    @Bind(com.jeremy.lychee.R.id.sub_topic_list)
    ListView mListView;
    ...
    void onClickErrorLayout() {
        mAdapter.clearData();
        requestAndShowSubTopics(1);
    }

    @Override
    protected void setContentView() {
        setContentView(com.jeremy.lychee.R.layout.activity_subscribed_topic);
    ...
    public static void startActivity(Context context) {
        Intent intent = new Intent(context, WeMediaSubscribedTopicsActivity.class);
    ...
    private void requestAndShowSubTopics(int start) {
        getMyTopicList(start, REQUEST_SIZE)
                .subscribeOn(Schedulers.io())
                .doOnNext(s -> {
                    if (s.size() >= REQUEST_SIZE) {
                        //request continue
                        requestAndShowSubTopics(start + 1);
                    }
                })
