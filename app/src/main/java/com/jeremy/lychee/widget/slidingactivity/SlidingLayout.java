...
public class SlidingLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (childCount > 2) {
            Logger.t(TAG).e("onMeasure: More than two child views are not supported.");
        }
        ...
        if (Build.VERSION.SDK_INT >= 11) { // HC
            ...
            if (lp.dimWhenOffset && mSlideOffset > 0) {
                ...
                if (cache != null) {
                    ...
                    Logger.t(TAG).e("drawChild: child view " + child + " returned null drawing cache");
                    result = super.drawChild(canvas, child, drawingTime);
                }
            } else {
                if (child.isDrawingCacheEnabled()) {
...
//            final int newLeft = Math.min(Math.min(left, leftBound), rightBound);
//            return newLeft;
            if (reverse)
                return Math.min(Math.min(left, leftBound), rightBound);
    ...
    static class SlidingPanelLayoutImplJB extends SlidingPanelLayoutImplBase {
        ...
        SlidingPanelLayoutImplJB() {
            try {
                ...
                Logger.t(TAG).e("Couldn't fetch getDisplayList method; dimming won't work right.", e);
            }
            try {
                mRecreateDisplayList = View.class.getDeclaredField("mRecreateDisplayList");
                mRecreateDisplayList.setAccessible(true);
            } catch (NoSuchFieldException e) {
                Logger.t(TAG).e("Couldn't fetch mRecreateDisplayList field; dimming will be slow.", e);
            }
        }
        ...
        public void invalidateChildRegion(SlidingLayout parent, View child) {
            if (mGetDisplayList != null && mRecreateDisplayList != null) {
                try {
                    ...
                    Logger.t(TAG).e("Error refreshing display list state", e);
                }
            } else {
