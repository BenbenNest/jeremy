...
public abstract class HeaderFooterRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public boolean isHeaderType(int type) {
        return (type >= HEADER_VIEW_TYPE_OFFSET && type < HEADER_VIEW_TYPE_OFFSET + VIEW_TYPE_MAX_COUNT);
    }

    public boolean isFooterType(int type) {
        return (type >= FOOTER_VIEW_TYPE_OFFSET && type < FOOTER_VIEW_TYPE_OFFSET + VIEW_TYPE_MAX_COUNT);
    }
    public boolean isContentType(int type) {
        return (type >= CONTENT_VIEW_TYPE_OFFSET && type < CONTENT_VIEW_TYPE_OFFSET + VIEW_TYPE_MAX_COUNT);
    }
